...
def genemask(dirpatientdb,cla,wra):
...
#        print 'visua dptail', topdir
    ...
    predictout_dir = os.path.join(dirpatientdb, lungmask)
    if not os.path.exists(predictout_dir):
        os.mkdir(predictout_dir)
    predictout_dir_bmp = os.path.join(predictout_dir,lungmaskbmp)
    if not os.path.exists(predictout_dir_bmp):
        os.mkdir(predictout_dir_bmp)
...
#    print preprob[0], listnamepatch
    ...
    for img in listbmpscan:
        ...
        imgcore=img[0:endnumslice]+'_'+str(slicenumber)+'.'+typei
        lungfilename=os.path.join(predictout_dir_bmp,imgcore)
        ill=0
...
#        cv2.destroyAllWindows()
        ...
        cv2.imwrite(lungfilename,closing)  
#    print dirpatientfdb
...
#                        print 'loop',scanumber
                ...
                genemask(namedirtopcf,classdirec,True)
