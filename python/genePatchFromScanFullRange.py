...
#            print fxs                       
            ...
            imgcoreScan=filename[0:endnumslice]+'_'+str(scanNumber)+'.'+typeid 
            imgcoreRoi=filename[0:endnumslice]+'_'+str(scanNumber)+'.'+typei 
...
def pavbg(namedirtopcf,dx,dy,px,py):
    ...
    lbmp   = os.listdir(patchpathc) #scan
    listbg = os.listdir(bgdirf) #bgdir lung
#    print patchpathc
    pxy=float(px*py) 
    for lm in listbg:
        ...
        endname=lm.find('.'+typei)
        core=lm[0:endname]+'.'+typeid
        nambmp=os.path.join(patchpathc,core)
...
#                                 print 'Image to write', min_val, max_val
...
#                                    print patchpath+nampa
#                                    recpo = Image.open(patchpath+nampa,'r')
#                                    recpo = cv2.imread(patchpath+nampa,cv2.IMREAD_ANYDEPTH)
...
#                                        tabi2 = cv2.equalizeHist(imgray)
                                    ...
                                    cv2.imwrite (patchNormpath+nampa, tabi2,[int(cv2.IMWRITE_PNG_COMPRESSION),0])
#                                    recpo = cv2.imread(patchNormpath+nampa,cv2.IMREAD_ANYDEPTH)
#                                    tarec=np.array(recpo)
#                                    min_val=np.min(tarec)
#                                    max_val=np.max(tarec) 
#                                    print 'Image recup', min_val, max_val
...
#        print n# list scan images
        slicescan=rsliceNum(n,'_','.'+typeid)
        #                    print(slns)
#        print n
        endname=n.find('.'+typeid)
        core=n[0:endname]+'.'+typei
#        print core
#        ooo
...
#                print lunglist
#                print slin
                slicelung=rsliceNum(core,'_','.'+typei)
...
#                    print np.shape(tabhc), np.shape(mask),slicelung
...
#                        print ttr
                        ...
                        if imagemax==None or max_val-min_val<20:
...
#                            print imagemax, i,j,slicenumber
                           
                            
#                            print 'patch', min_val,max_val

                            nbp+=1
                            nampa='/'+label+'/'+loca+'/'+f+'_'+iln+'_'+str(nbp)+'.'+typeid
                            imgray =np.array(crorig)
                            imgray=imgray.astype('uint16')
#                            min_val=np.min(imgray)
#                            max_val=np.max(imgray)
#
#                            print 'imgray', min_val,max_val
#                            n, bins, patches = plt.hist(imgray)
#                            plt.show()
...
#                            imgray =np.array(crorig)
##                            orign = Image.open(patchpath+nampa)
#                            orignnonorm = cv2.imread(patchpath+nampa,cv2.IMREAD_ANYDEPTH)
####                            torign = np.array(orign) 
####                            print orign
#                            min_val=np.min(orignnonorm)
#                            max_val=np.max(orignnonorm)
##                            
#                            print 'recup orig ', min_val, max_val
#                            ooo
                            if normiInternal:
                                        tabi2 = normi(imgray) 
                            else:
...
#                            print 'tabi2', min_val, max_val
#                            ooo
#                            print tabi2
                            cv2.imwrite (patchNormpath+nampa, tabi2,[int(cv2.IMWRITE_PNG_COMPRESSION),0])
#                            origno = Image.open(patchpath+nampa)   
#                            origno = cv2.imread(patchNormpath+nampa,cv2.IMREAD_ANYDEPTH)
#                            min_val=np.min(origno)
#                            max_val=np.max(origno)
#                            print 'recup orig norm', min_val, max_val
#                            ooo
#                            print origno
#                            n, bins, patches = plt.hist(origno)
#                            plt.show()
#                            ooo
