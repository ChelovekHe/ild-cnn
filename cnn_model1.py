...
def get_model(input_shape, output_shape, params):
    ...
    print(' pool_siz : ', pool_siz)
    
    # Initialization of the model
    model = Sequential()

    a = params['k']*get_FeatureMaps(1, params['fp'])
    print ('nb_filters : ', a)    # yields 16
    print ('LeakyRELU parameters : ', params['a'])
    print ('input shape : ', input_shape[1:])

    # Add convolutional layers to model
    # model.add(Convolution2D(params['k']*get_FeatureMaps(1, params['fp']), 2, 2, init='orthogonal', dim_ordering='th', activation=LeakyReLU(params['a']), input_shape=input_shape[1:]))
    
    # model.add(Convolution2D(16, 2, 2, init='orthogonal', dim_ordering='th', activation=LeakyReLU(0.3), input_shape=(1, 28, 28)))
    
    model.add(Convolution2D(16, 2, 2, init='orthogonal', border_mode='valid', input_shape=(1, 28, 28)))
    ...
    for i in range(2, 6):
        model.add(Convolution2D(16, 2, 2, init='orthogonal', border_mode='valid'))
